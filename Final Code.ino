...
Lock mechanism, password masking, 
retry limit 
...
const char* scriptURL = "https://script.google.com/macros/s/AKfycbzlx2p5zxmBZBiUiijv
8QaU6M3mwbz7JqBzesgCcmC2yXEMbwZRHgqj5rMiA373uB48A/exec"; 
...
void loop() { 
  if (systemLocked) { 
    ...
    if (!lastUserWasAdmin) { 
      ...
      while (true) { 
        Serial.println("Admin: Enter ID..."); 
        displayFormattedMessage("Admin ID:"); 
        String enteredID = getUserInput(false); 
         
        bool isAdmin = false; 
        for (int i = 0; i < totalUsers; i++) { 
          if (users[i].id == enteredID && users[i].isAdmin) { 
            isAdmin = true; 
            break; 
          } 
        } 
        ...
        if (isAdmin) { 
          Serial.println("Admin Found. Enter Reset Code:"); 
          displayFormattedMessage("Enter Code:"); 
          String enteredCode = getUserInput(true); 
           
          if (enteredCode == resetCode) { 
            Serial.println("System Unlocked by Admin!"); 
          ...
          Serial.println("Not an Admin!"); 
          displayFormattedMessage("Invalid Admin"); 
        } 
      } 
    } 
  ...
  if (ir_value == LOW) {   
    ...
    while (passwordAttempts < 6) { 
      ...
      if (enteredPassword == correctPassword) { 
        ...
        EServo.attach(SERVO_PIN);   
        EServo.write(90);   // Rotate to open 
        delay(5000);         
        EServo.write(0);  
